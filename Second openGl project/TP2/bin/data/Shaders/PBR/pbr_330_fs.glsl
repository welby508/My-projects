...
// fonction qui calcule un modèle d'illumination de type pbr avec brdf de cook-torrance avec une position de lumiere donner en argument
vec3 brdf_cook_torrance(vec3 light_pos)
{
  ...
  float light_distance = length(light_pos - surface_position);
...
void main()
{
  ...
  if (lights_on == 0)
  {
   color = vec3(material_color_ambient);
  }

  if (lights_on == 1)
  {
    color = brdf_cook_torrance(side_light_position);
  }

  if (lights_on == 2)
  {
    color = brdf_cook_torrance(spot_light_position);
  }

  if (lights_on == 3)
  {
    color = brdf_cook_torrance(spot_light_position) +  brdf_cook_torrance(side_light_position);
  }
